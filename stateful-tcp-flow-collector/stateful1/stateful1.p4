    ...
    inout pna_main_output_metadata_t ostd)
{
    ...
    table ct_tcp_table {
        key = {
            SelectByDirection(is_net_port(istd.input_port),
                               hdr.ipv4.srcAddr,
                               hdr.ipv4.dstAddr):
                               exact @name("ipv4_addr_0") @type("ipv4");
            SelectByDirection(is_net_port(istd.input_port),
                               hdr.ipv4.dstAddr,
                               hdr.ipv4.srcAddr):
                               exact @name("ipv4_addr_1") @type("ipv4");
            hdr.ipv4.protocol : exact;
            SelectByDirection(is_net_port(istd.input_port),
                               hdr.tcp.srcPort,
                               hdr.tcp.dstPort):
                               exact @name("tcp_port_0") @type("be16");
            SelectByDirection(is_net_port(istd.input_port),
                               hdr.tcp.dstPort,
                               hdr.tcp.srcPort):
                               exact @name("tcp_port_1") @type("be16");
        }
        actions = {
